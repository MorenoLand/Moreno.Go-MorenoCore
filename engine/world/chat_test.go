...
func TestLuaChatHookCanCancelMessage(t *testing.T) {
	...
	if err := runtime.LoadString(`RegisterPlayerEvent(18, function(event, player, msg) if player:GetName() == "Tester" and msg == "blocked" then return false end end)`); err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range []struct {
		...
		{message: "blocked", cancelled: true},
		{message: "allowed", cancelled: false},
	} {
		values, err := state.triggerPlayerEventValues(context.Background(), scripting.PlayerEventChat, state.luaPlayer(), test.message, chatSay, 7)
		if err != nil {
			t.Fatalf("message %q hook failed: %v", test.message, err)
		}
		if luaCancelled(values) != test.cancelled {
			t.Fatalf("message %q cancelled=%v, want %v", test.message, luaCancelled(values), test.cancelled)
		}
	}
}
